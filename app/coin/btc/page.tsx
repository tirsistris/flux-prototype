"use client";
// app/coin/btc/page.tsx — Coin detail, ported from screen-coin.jsx. BTC is
// hardcoded on purpose (coin/[id] parametrization is phase 4). Buy → /buy;
// back → /home. Sell stays an inert control (honest dead button for now).
  ...
  return (
    <PhoneFrame scroll footer={footer}>
      <div className="fl-detail-head">
        <button className="fl-roundbtn sm" onClick={() => router.push("/home")}>{Ico.chevL("rgba(255,255,255,0.85)", 20)}</button>
        <div className="fl-detail-coin">
          <CoinAvatar id="btc" size={28} />
      ...
      <div style={{ marginTop: 16 }}>
        <AreaChart vals={FLUX.curve} w={342} h={140} label={eur(103455.44)} id="coin" />
      </div>

      <div className="fl-stat2-grid" style={{ marginTop: 20 }}>
        <Stat2 label="Market cap" value={"2,07 T €"} />
        <Stat2 label="24h Volume" value={"38,4 Mrd €"} />
        <Stat2 label="24h High" value={eur(108240)} />
        <Stat2 label="24h Low" value={eur(101920)} />
      </div>
      ...
      <div className="fl-card fl-holding">
        <CoinAvatar id="btc" size={42} />
        <div style={{ display: "flex", flexDirection: "column", gap: 3 }}>
          <span style={{ fontSize: 14.5, fontWeight: 600, color: "#fff" }}>Your {c.name}</span>
          <span style={{ fontSize: 12.5, color: "#8A91A3" }}>{fmtNum(holdAmt, 4)} {c.sym}</span>
        </div>
      ...
      <p className="fl-about-text">
        ...
        a simple first step into crypto. <span className="fl-about-link">Read more</span>
      </p>
    </PhoneFrame>
  );
}
